...
kubectl ${ACTION} --context="kind-${CLUSTER2_NAME}" -n "${ISTIO_NAMESPACE}" -f /tmp/grafana.yaml
...
if [[ $ACTION != 'delete' ]]; then
while : ; do
  GRAFANA_ADDR=$(kubectl get --context kind-${CLUSTER2_NAME} -n $ISTIO_NAMESPACE \
    services/grafana-endpoint-service -o jsonpath='{ .status.loadBalancer.ingress[0].ip}')

  if [[ ! -z $GRAFANA_ADDR ]]; then
      echo "Public IP address ${GRAFANA_ADDR} is now available"
      break
  fi
  echo 'Waiting for Grafana public IP address to be available...'
  sleep 3
done
fi

if [[ "$ACTION" == 'apply' ]]; then
  # Setup kiali service account and secret in the config cluster
  kubectl create sa kiali-service-account -n $ISTIO_NAMESPACE --context="kind-${CLUSTER2_NAME}"

  # Setup secret for the account
  istioctl x create-remote-secret --service-account kiali-service-account \
    --context="kind-${CLUSTER2_NAME}" -n "${ISTIO_NAMESPACE}" --name kiali | \
    kubectl apply -n "${ISTIO_NAMESPACE}" --context="kind-${CLUSTER1_NAME}" -f -
else
  kubectl delete sa kiali-service-account -n $ISTIO_NAMESPACE --context="kind-${CLUSTER2_NAME}"
  kubectl delete -n "${ISTIO_NAMESPACE}" --context="kind-${CLUSTER1_NAME}" secret istio-remote-secret-kiali
fi

# Setup kiali service account role and role binding in cluster2
kubectl ${ACTION} --context="kind-${CLUSTER2_NAME}" -n "${ISTIO_NAMESPACE}" -f - <<EOF
---
...
# Now setup kaili
echo "Setting up kaili confguration in ${CLUSTER1_NAME}"
...
data:
  config.yaml: |
    ...
    external_services:
      istio:
        url_service_version: http://istiod.${ISTIO_NAMESPACE}:15014/version
        component_status:
          enabled: false
      prometheus:
        url: http://$PROMETHEUS_ADDR:20002
      grafana:
        url: http://$GRAFANA_ADDR:20004
      custom_dashboards:
        enabled: true
    identity:
      cert_file: ""

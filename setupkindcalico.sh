...
function waitForPods() {
  ...
  while : ; do
    res=$(kubectl wait --context "kind-${CLUSTERNAME}" -n ${ns} pod \
      -l ${lb} --for=condition=Ready --timeout=20s 2>/dev/null ||true)
...
waitForPods kube-system k8s-app=calico-kube-controllers
waitForPods kube-system k8s-app=calico-node
...
# Wait for metallb to be ready, both controller and speak use app=metallb label
waitForPods metallb-system app=metallb
...
echo "Kubernetes cluster with Calico SNI ${CLUSTERNAME} was created successfully!"
